...
type RockonDetails struct {
	...
	CustomConfig     string               `json:"custom_config,omitempty"`      // custom configuration object that a special install handler of this Rock-on expects
}

type UISlug struct {
	Slug string `json:"slug"` // link to webui becomes ROCKSTOR_IP:PORT/gui with slug value gui
}
...
type Container struct {
	...
	Opts         []Option                  `json:"opts"`          // Array of Option objects that represent container options such as --net=host etc.
	CmdArguments []CmdArgument             `json:"cmd_arguments"` // Array of CmdArgument objects that represent arguments to pass to the 'docker run' command.
	Environment  map[string]EnvironmentVar `json:"environment"`   // Map of environment variable names to EnvironmentVar objects, representing the value
	Devices      map[string]Device         `json:"devices"`       // Map of device paths to Device objects, to be passed through to the container
}
...
type Volume struct {
	...
	MinSize     int    `json:"min_size"`    // suggested minimum size of the Share, in KB
}
